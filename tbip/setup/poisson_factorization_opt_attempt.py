...
def get_expected_data_log_likelihood(counts,
  ...
  negative_rate = -np.sum(expected_document @ expected_topic)#-np.sum(np.matmul(expected_document, expected_topic))
...
def get_auxiliary_entropy(normalizer, counts):
  h5 = tb.open_file('/workspace/pranav/tbip/data/tweets_cong_115_116/normalizer.h5', 'r')
  normalizer = h5.root.data
  skip = 10000
  num_documents, num_words = counts.shape
  total = 0.0
  for i in range(0, num_documents, skip):
      total = total + np.sum(counts[i:i+skip] * normalizer[i:i+skip, :])
  return total#np.sum(counts * np.log(normalizer))
...
def update_variational_document_shape(counts,
  ...
  geometric_topic = geometric_gamma_mean(variational_topic_shape,
                                         variational_topic_rate)
  print(geometric_topic.shape)
  geometric_document = geometric_gamma_mean(variational_document_shape,
                                            variational_document_rate)
  print(geometric_document.shape)
  partial_expected_counts = []
  for i in range(0, num_documents, skip):
      nc = counts[i:i+skip].toarray() / normalizer[i:i+skip, :]
      print(nc.shape)
      print(geometric_document[i:i+skip].shape)
      print(geometric_topic[i:i+skip].T.shape)
      partial_expected_counts.append(geometric_document[i:i+skip] * (nc @ geometric_topic[i:i+skip].T))
  #expected_counts = geometric_document * np.matmul(
      #normalized_counts,
      #geometric_topic.T)
  expected_counts = np.concatenate(partial_expected_counts, 0)
  print(expected_counts.shape)
  del partial_expected_counts
...
def update_variational_topic_shape(counts,
  ...
  skip = 10000
  num_documents, num_words = counts.shape
  geometric_topic = geometric_gamma_mean(variational_topic_shape, variational_topic_rate)
  geometric_document = geometric_gamma_mean(variational_document_shape, variational_document_rate)
  partial_expected_counts = []
  for i in range(0, num_documents, skip):
      nc = counts[i:i+skip].toarray() / normalizer[i:i+skip, :]
      partial_expected_counts.append(geometric_topic[i:i+skip] * (geometric_document[i:i+skip].T @ nc))
  #expected_counts = geometric_document.multiply(normalized_counts @ geometric_topic.T)
  ...
  return variational_topic_shape#.todense()
...
def get_normalizer(variational_topic_shape,
  ...
  geometric_document = sparse.csr_matrix(geometric_gamma_mean(variational_document_shape,
                                                  variational_document_rate).astype('float32'))
  geometric_topic = sparse.csc_matrix(geometric_gamma_mean(variational_topic_shape,
                                                 variational_topic_rate).astype('float32'))
  l, m, n = geometric_document.shape[0], geometric_document.shape[1], geometric_topic.shape[1]
  f = tb.open_file('/workspace/pranav/tbip/data/tweets_cong_115_116/normalizer.h5', 'w')
  filters = tb.Filters(complevel=5, complib='blosc')
  out = f.create_carray(f.root, 'data', tb.Float32Atom(), shape=(l, n), filters=filters)

  bl = 10000
  for i in range(0, l, bl):
      out[i:min(i+bl, l),:] = (geometric_document[i:min(i+bl, l), :].dot(geometric_topic)).toarray()
  #normalizer = geometric_document.dot(geometric_topic)
  print('normalizer computed and stored')
  f.close()
  #return normalizer
...
def main(argv):
  ...
  counts = sparse.load_npz(os.path.join(data_dir, "counts.npz")).astype('int32')
  ...
  for step in range(FLAGS.max_steps):
    ...
    #normalizer = 
    get_normalizer(variational_topic_shape,
                                variational_topic_rate,
                                variational_document_shape,
                                variational_document_rate)
    normalizer = None
