...
Let y_{dv} denote the counts of word v in document d. Let x_{dk} refer to the 
ideal point of the author of document d for topic k. Then we model:

theta, beta ~ Gamma(alpha, alpha)
x, eta ~ N(0, 1)
y_{dv} ~ Pois(sum_k theta_dk beta_kv exp(x_dk * eta_kv).
...
Note that x and eta are real-valued vectors. x is a K-valued vector (ideal points for each topic for each author) and eta is a V-valued vector (value of each word for each topic). 
...
class TBIP(torch.nn.Module):
  ...
  def __init__(self, 
    ...
    self.ideological_topics = VariationalFamily(
        ...
        'normal',
        [num_topics, num_words])
    self.ideal_points = VariationalFamily(
        device,
        'normal',
...
def log_static_features(model, summary_writer, vocabulary, author_map, step):
  ...
  ideal_point_list = print_ideal_points(
      model.ideal_points.location.cpu().detach().mean(axis=1),
      author_map)
...
def main(argv):
  ...
  save_dir = os.path.join(source_dir, "tbip-pytorch-fits-issue-specific")
